...
class MyTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        kms_client = boto3.client("kms", endpoint_url='http://localstack:4566')
        ...
        print(signature)
        dyn_client = boto3.client("dynamodb", endpoint_url='http://localstack:4566')
        try:
    ...
    def test_something(self):
        print(f'ansible-playbook -vvv --connection=local --inventory 127.0.0.1, --limit 127.0.0.1 --extra-vars "key_id={self.key_id} endpoint_url=--endpoint-url\=http://localstack:4566" {os.path.dirname(__file__)}/../src/ansible.yml')
        os.system(f'ansible-playbook --connection=local --inventory 127.0.0.1, --limit 127.0.0.1 --extra-vars "key_id={self.key_id} endpoint_url=--endpoint-url\=http://localstack:4566" {os.path.dirname(__file__)}/../src/ansible.yml')
